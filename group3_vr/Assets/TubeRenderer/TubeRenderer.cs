/*
	Copyright 2016-2017 Sixth Sensor.
	All rights reserved.
	http://sixthsensor.dk
...
public class TubeRenderer : MonoBehaviour
{
	...
	void UpdateRotations()
	{
		...
		for( int p=0; p<points.Length-1; p++ ){
			if( _lengths[p] > 0 ){
				...
				// We assume the neighbor direcction is not zero. Not perfect, but fast.
				if( p == 0 ) _directions[p] = _directions[p+1] / _lengths[p+1];
				else _directions[p] = _directions[p-1];
			}
		}
		...
		if( firstFrame ){
			up = _directions[0].x == 0 && _directions[0].z == 0 ? Vector3.right : Vector3.up;
		} else {
			up = _pastBeginUp;
		}
	...
	{
		// Update array length.
		if( _steepnessAngles == null || _steepnessAngles.Length != _points.Length ) _steepnessAngles = new float[ _points.Length ];
		...
		_steepnessAngles[_points.Length-1] = -Mathf.Atan2( radiusDiffs[_points.Length-2], _lengths[_points.Length-1] ) * Mathf.Rad2Deg;
	...
	void UpdateUVs()
	{
		...
		case NormalMode.Hard:
			for( int p=0; p<points.Length-1; p++ ){
				...
				for( int e=0; e<_edgeCount; e++ ){
					...
					_uvs[ uv++ ] = new Vector2( u, v );
					_uvs[ uv++ ] = new Vector2( nextU, v );
					_uvs[ uv++ ] = new Vector2( nextU, nextV );
					_uvs[ uv++ ] = new Vector2( u, nextV );
				}
			}
			break;
		case NormalMode.HardEdges:
		...
		if( _caps == CapMode.Both || _caps  == CapMode.Begin ){
			...
			_uvs[ uv++ ] = new Vector2( u, v ); // center
		}
